...
function fetchCourseDetail(courseId){
	var listner1 = db_firestore.collection("courseDetailsCollection").doc(courseId)
		.onSnapshot(function (doc) {
			console.log("courseDetailsCollection:",doc.data());
			courseDetailData = doc.data();
			var data = doc.data();
			$(".courseName").text(data.title);
			$(".courseDetail").text(data.description);
			...
			console.log("price:"+new Number(data.pricingPlans[0].price).toLocaleString());
			$(".price").text(currencySymbol + new Number(Math.round(data.pricingPlans[0].price*currencyMultiple/10)*10).toLocaleString()+" onwards");
			...
			for(var i=0;i<tasks.length;i++){
				...
				lessonCategory[tasks[i].taskGroup].push(tasks[i]);
			}
			...
			for( var key in lessonCategory ) {
			  ...
			  html += '<div class="card">'
								+'<div class="card-header" role="tab" id="headingOne">'
									+'<h5 class="mb-0">'
										+'<a data-toggle="collapse" href="#collapse'+counter+'" aria-expanded="false" aria-controls="collapseOne"><i class="indicator ti-plus"></i> '+key+'</a>'
									+'</h5>'
								+'</div>'
								+'<div id="collapse'+counter+'" class="collapse" role="tabpanel" aria-labelledby="headingOne" data-parent="#accordion_lessons">'
									+'<div class="card-body">'
										+'<div class="list_lessons">'
											+'<ul>';
												
			  for(var i=0;i<value.length;i++){
				  html += '<li><a class="txt_doc">'+value[i].taskTitle+'</a></li>';
			  }
			  html+='</ul>'
						+'</div>'
					+'</div>'
